...
echo "Convert HTML to docx and put them in the output-latex/ directory"

[[ -d ./output-latex/ ]] || mkdir ./output-latex/ || exit 111
...
for filename in ${all_content}; do
  ...
  docx_file="output-latex/${human_name}.docx"

  echo "Generating '${docx_file}' using ${base_name}"

  # convert MathML to LaTeX text
  latex_temp_file="${filename}-latex"
  python script/mathml2latex/mathml2latex.py ${filename} >${latex_temp_file}

  ./script/html-to-docx.bash ${latex_temp_file} "${docx_file}"
done